...
* The status bar's dot says how many nodes are ready and which Kubernetes version, and opens a window on the cluster: each node's readiness, age, pods, what its pods have requested of what it has, what it uses now and any memory, disk or PID pressure; how many deployments and workspaces have each health, a click away from the list; and whether kso's scheduler keeps up. Read when it opens, never polled
