...
* The job queue: a job whose worker died while running it - its pod went away - was left reserved and never ran; after an hour it is moved to the failed jobs with the reason, to be started again with `spark queue:retry` if it should. Failed jobs are removed after a month
...
* The previous container's log could show the kubelet's "unable to retrieve container logs" cut after its first word, as if that word were the time
...
* Audit trail: every change to kso's setup is recorded with who made it, from where, and the fields before and after - secrets only as changed. Deploy, terminate, pause, commands run in a pod, scans, sign-ins and sign-outs are recorded as actions. A change and its record are written together, and the trail is kept for 90 days. Read it under Audit Trail in the menu, or as the History of a workspace or deployment; `GET /audit_events` for the API
...
* The menu is a rail of icons on every screen, opening over the page on hover, with the counts on the icons. It stood open at 256px on every screen, because it was set up with Vuetify 2's names for it
* Workspaces: more room on the row. The namespace sits under the name, the url drops `https://`, the status no longer wraps to three lines, and logs, migration jobs and history moved into the row's menu
* Deployments: the same. The namespace sits under the name, and migration jobs, history and Delete moved into the row's menu - Delete was a red button beside the rest, and Settings was already a click on the name
* The status is worked out every minute, like the health, so a resource deleted behind kso's back is noticed instead of standing as Synced until somebody deploys or presses refresh. It is read off one round of lists - one per kind, whatever the installation's size - rather than a call per step per deployment, and the refresh button on a workspace of five deployments or more uses the same
...
* Deployments: "Why?" on one that is Degraded, Progressing or Missing says what kso can see is wrong - a tag the registry does not have, a missing pull secret, a memory limit too low, a new version that crashes where the old one ran, a failing readiness probe, a manifest the cluster refused, a failed migration - with what it was read off and a button that fixes it, such as rolling back. A rule that cannot settle it says so rather than guess. Behind a GKE Gateway it also asks each pod for the health check path, as the load balancer does, and says when it answers anything but 200 - a path behind basic auth answers 401. "Why?" is in the health's menu on every deployment row too, and a failed migration opens its log. A workspace's page counts its Degraded deployments beside Deployments in the side menu
* A deployment's page has its Kubernetes Logs, and Migration Logs when its specification migrates, as a workspace's has
* The status bar's dot shows the Kubernetes version in it when every node runs the same one, says how many nodes are ready, and opens a window on the cluster: each node's readiness, age, pods, what its pods have requested of what it has, what it uses now and any memory, disk or PID pressure; how many deployments and workspaces have each health, a click away from the list; and whether kso's scheduler keeps up. Read when it opens, never polled
* Gateways: "In the cluster" lists the cluster's Gateways beside kso's. For one kso has, it says what differs - the class, the addresses, a listener kso would not write. One kso does not have can be taken over: the dialog says which domains are linked to it and what kso's next Deploy would remove from its listeners and add, the name has to be typed, and nothing is applied until it is deployed
* Domains: "In the cluster" lists the cluster's cert-manager Certificates beside kso's domains, with what differs for kso's own. One kso does not have can be taken over as a domain, linked to the kso gateway that listens for it: the dialog says what kso's next apply would change - above all a secret it would rename - the name has to be typed, and nothing is applied. Operators' certificates for names inside the cluster are kept apart
* kso marks the namespaces, gateways and certificates it deploys with its own installation (`4spaces.kso/installation`), so two kso's sharing a cluster can tell theirs apart: another kso's are shown as such in "In the cluster" and cannot be taken over. The cluster window lists every namespace by whose it is - this kso's, with a link to the workspace, another kso's, Kubernetes' own, or nobody's - with its pods. What was deployed before the upgrade is marked on its next deploy
