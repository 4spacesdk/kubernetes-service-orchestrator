...
export interface DeploymentSpecificationUpdateClusterRoleRulesDialog_Input {
...
const props = defineProps<{ input: DeploymentSpecificationUpdateClusterRoleRulesDialog_Input, events: DialogEventsInterface }>();
...
function render() {
    ...
    Api.deploymentSpecifications().get()
        ...
        .include('deployment_specification_cluster_role_rule')
        .find(value => {
            rows.value = value[0].deployment_specification_cluster_role_rules
                ?.map(clusterRoleRule => {
                    return {
                        apiGroup: clusterRoleRule.api_group ?? '',
                        resource: clusterRoleRule.resource ?? '',
                        verbs: clusterRoleRule.verbs ?? '',
                    }
                }) ?? [];
...
function onCreateBtnClicked() {
    ...
    bus.emit('deploymentSpecificationUpdateClusterRoleRule', {
        clusterRoleRule: newItem,
...
function onSaveBtnClicked() {
    isSaving.value = true;
    const api = Api.deploymentSpecifications().updateClusterRoleRulesPutById(props.input.deploymentSpecification.id!);
        ...
        <v-card
            ...
            <v-card-title>
                <div class="d-flex w-100">
                    <span class="my-auto">Cluster Role Rules</span>
