...
import {WorkspaceTemplate, DeploymentSpecification} from "@/core/services/Deploy/models";
import {WorkloadTypes} from "@/constants";

export interface WorkspaceTemplateUpdateDeploymentSpecificationsDialog_Input {
...
const props = defineProps<{ input: WorkspaceTemplateUpdateDeploymentSpecificationsDialog_Input, events: DialogEventsInterface }>();
...
function render() {
    ...
    Api.workspaceTemplates().get()
        ...
        .include(`workspace_template_deployment_specification?include=${encodeURIComponent('deployment_specification,k_native_min_scale_schedule')}`)
        .find(value => {
            rows.value = value[0].workspace_template_deployment_specifications
...
function onCreateItemBtnClicked(createItem: CreateItem) {
    ...
    switch (createItem.deploymentSpecification.workload_type) {
        ...
        case WorkloadTypes.DaemonSet:
            bus.emit('workspaceTemplateUpdateDeploymentSpecification', {
                settings: newItem,
                onSaveCallback: () => {
...
function onEditRowKNativeMinScaleSchedulesClicked(row: Row) {
    bus.emit('workspaceTemplateUpdateDeploymentSpecificationKNativeMinScaleSchedules', {
...
function onSaveBtnClicked() {
    isSaving.value = true;
    const api = Api.workspaceTemplates().updateDeploymentSpecificationsPutById(props.input.workspaceTemplate.id!);
